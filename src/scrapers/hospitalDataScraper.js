...
async function hospitalDataScraper(
  hospitalUrl = "https://www.vaidam.com/hospitals/burjeel-hospital-abu-dhabi"
) {
  ...
  try {
    ...
    let hospitalData = await page.evaluate(async () => {
      ...
      const food = [
        "Diet on Request",
        "International Cuisine/ Multi Cuisine",
        "Restaurant",
      ];
      const transport = [
        "Air ambulance",
      ...
      facility_comforts = facility_comforts.filter((item) =>
        comforts.includes(item)
      );
      facility_treatment = facility_treatment.filter((item) =>
        treatment.includes(item)
      );
      facility_language = facility_language.filter((item) =>
        language.includes(item)
      );
      facility_food = facility_food.filter((item) => food.includes(item));
      facility_transport = facility_transport.filter((item) =>
