            ...
            if (! hospitalData ?. clinicspotsId) {
                if (hospitalData ?. name) {
                    if (hospitalData ?. city_name) {
                        const result = await HospitalPageUrls.findOne({url: hospitalData.url});
                        ...
                        let categories = departments ?. map(async (department) => {
                            ...
                            const mappedDepartment = await CategoryMappingColl.findOne({VaidamDepartment: department});
                            if (mappedDepartment) {
                        ...
                        await HospitalDataColl.updateOne({
                            ...
                            $set: {
                                clinicspotsId: "This hospital cannot be saved!"
                            }
                        });
                        await HospitalErrorColl.insertOne({hospitalId: hospitalData._id, errorMessage: "This Hospital's CITY_NAME not present"});
                    }
                } else {
                    await HospitalDataColl.updateOne({
                        url: hospitalData.url
                    ...
                    await HospitalErrorColl.insertOne({hospitalId: hospitalData._id, errorMessage: "This Hospital's NAME not present."});
                }
            }
        }
        if (err) 
...
async function mapDoctors(hospitalData) {
    ...
    let doctors = hospitalData.doctors.map(async (doctorUrl) => {
        let result = await DoctorDataColl.findOne({url: doctorUrl});
    ...
    Promise.all(doctors).then(async (doctorsArr) => {
        let doctors = doctorsArr.filter((doctorId) => {
            return(doctorId !== undefined && doctorId !== null && doctorId !== "This doctor cannot be saved!" && typeof doctorId === "number");
        });
        doctors = doctors || [];
        // null check
        // removing any duplicity
        ...
        const mappedAmenities = {
            ...
            "Special offer for group stays": "Group Stays"
        };
        delete hospitalData.clinicspotsId;
        if (isNaN(parseInt(hospitalData.beds, 10))) {
            delete hospitalData.beds;
        }
        hospitalData.facilities = hospitalData.facilities.map((amenity) => {
...
async function sendRequest(payload) {
    const HospitalErrorColl = db.collection("hospitalError");
    const HospitalDataColl = db.collection("hospitalData");
    await axios.post(apiUrl, payload).then(async (response) => {
        ...
        if (typeof hospitalId === "number") {
            await HospitalDataColl.updateOne({
                ...
                $set: {
                    clinicspotsId: hospitalId
                }
            });
            console.log("SUCCESS => Hospital Saved Successfully in CLINICSPOTS", "ID => ", hospitalId);
        } else 
