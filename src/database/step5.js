        ...
        while (await cursor.hasNext()) {
            ...
            if (! doctorData ?. clinicspotsId) {
                if (doctorData ?. first_name) {
                    if (doctorData ?. city_name) { // if (doctorData.city_name !== "Balcova" && doctorData.city_name !== "Cankaya")
                        mapSpecialization(doctorData);
                    } else if (doctorData ?. hospitalUrl) {
                        const hospitalData = await HospitalDataColl.findOne({url: doctorData.hospitalUrl});
                        ...
                        if (cityName) {
                            ...
                            await DoctorDataColl.updateOne({
                                ...
                                $set: {
                                    clinicspotsId: "This doctor cannot be saved!"
                                }
                            });
                            await DoctorErrorColl.insertOne({doctorId: doctorData._id, errorMessage: "This Doctor's City and hospital's city not present"});
                        }
                    } else {
                        await DoctorDataColl.updateOne({
                            url: doctorData.url
                        ...
                        await DoctorErrorColl.insertOne({doctorId: doctorData._id, errorMessage: "This Doctor's CITY_NAME and HOSPITAL URL not present"});
                    }
                } else {
                    await DoctorDataColl.updateOne({
                        url: doctorData.url
                    ...
                    await DoctorErrorColl.insertOne({doctorId: doctorData._id, errorMessage: "This Doctor's FIRST_NAME not present."});
                }
            }
        }
        if (err) 
...
async function mapSpecialization(doctorData) {
   ...
   */
    const services = doctorData ?. services ?. map(async (service) => {
        const mappedService = await SpecializationMappingColl.findOne({VaidamSpecialization: service});
        if (mappedService) {
            return mappedService ?. Specialization1;
        }
        return;
    });
    let specializations = doctorData ?. specializations ?. map(async (specialization) => {
        const mappedSpecialization = await SpecializationMappingColl.findOne({VaidamSpecialization: specialization});
...
async function sendRequest(payload) {
    const DoctorErrorColl = db.collection("doctorError");
    const DoctorDataColl = db.collection("doctorData");
    await axios.post(apiUrl, payload).then(async (response) => { // console.log(response);
        ...
        if (typeof doctorId === "number") {
            await DoctorDataColl.updateOne({
                ...
                $set: {
                    clinicspotsId: doctorId
                }
            });
            console.log("SUCCESS => Doctor Saved Successfully in CLINICSPOTS", "ID => ", doctorId);
        } else 
            throw new Error("Id was not number");
